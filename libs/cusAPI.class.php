...
/**
 * Initialization Class for ContactUs.com API (suffix according to plugin _WOO)
 * @api
 * @version 0.4
...
class cUsComAPI_WOO {
     ...
     * @api
     */
    var $enviroment = cUsWOO_API_ENV;
    var $wpApiAccountKey = cUsWOO_API_ACC;
    var $wpApiKey = cUsWOO_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: wpwoo|1.1.2|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            $strCURLOPT .= '&Promotion_Code=WPWOO';
            $strCURLOPT .= '&Version=wpwoo|1.1.2';
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                ...
                'X-ContactUs-Signature: wpwoo|1.1.2|' . $this->getIP()
            ));
            $content = curl_exec($ch);
            curl_close($ch);
    ...
    public function getGoals() {
        ...
        $aryGoals = array(
			...
			'None, I just want a WooCommerce Newsletter SignUp Form on my site that sends to my email',
			'Other',
		);
    ...
    public function resetData() {
        ...
        delete_option('cUsWoo_settings_form_keys');
        delete_option('cUsWoo_settings_inlinepages');
        delete_option('cUsWoo_settings_tabpages');
        delete_option('cUsWoo_settings_userCredentials');
        delete_option('cUsWoo_settings_default_deep_link_view');
        delete_option('cUsWoo_settings_form_id');
        delete_option('cUsWoo_settings_FormByPage');

        cUsWoo_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUsWoo_inline_shortcode_cleaner(); //REMOVE SHORTCODES
        cUsWoo_deleteAllCatSettings(); //REMOVE CUSTOM SETTINGS BY CATEGORY
